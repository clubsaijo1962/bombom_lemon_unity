...
{
    /// <summary>
    /// チームバトルモード：ゲーム進行画面のシーンビルダー。
    /// チームA/B に分かれた回答者・最終決定者・予想者・観戦者の役割に対応した UI を構築する。
    /// </summary>
    public static class TeamGameSceneBuilder
    {
        ...
        static readonly Color TeamAColor   = new(0.85f, 0.90f, 1.00f, 0.90f);
        static readonly Color TeamBColor   = new(1.00f, 0.88f, 0.88f, 0.90f);
        ...
        public static void Build()
        {
            ...
            // ── カード: 1020×1640, center y=-88 ──
            // card local: top=+820, bottom=-820
            ...
            cardR.sizeDelta = new Vector2(1020f, 1640f);
            cardR.anchoredPosition = new Vector2(0f, -88f);

            // ── タイトル行 ──
            MakeLabel(cardGO.transform, "Title", "チームバトル ゲーム",
                new Vector2(0.5f, 0.5f), new Vector2(0f, 800f), new Vector2(960f, 64f),
                52f, TextPrimary, FontStyles.Bold, jpFont);

            // ── 自分の役割バッジ ──
            var myRoleTmp = MakeLabel(cardGO.transform, "MyRole", "🎯 あなたは予想者",
                new Vector2(0.5f, 0.5f), new Vector2(0f, 742f), new Vector2(860f, 48f),
                38f, TextMuted, FontStyles.Bold, jpFont);

            MakeSeparator(cardGO.transform, 712f);

            // ── 役割発表 ──
            var answererNameTmp = MakeLabel(cardGO.transform, "AnswererName", "回答者：〇〇",
                new Vector2(0.5f, 0.5f), new Vector2(0f, 672f), new Vector2(920f, 46f),
                36f, TextPrimary, FontStyles.Normal, jpFont);

            var deciderNameTmp = MakeLabel(cardGO.transform, "DeciderName", "最終決定者：△△",
                new Vector2(0.5f, 0.5f), new Vector2(0f, 622f), new Vector2(920f, 46f),
                36f, TextPrimary, FontStyles.Normal, jpFont);

            // ── ガイドテキスト ──
            var guideTmp = MakeLabel(cardGO.transform, "Guide",
                "回答を参考に、最終決定者の秘密の数字を予想しよう！",
                new Vector2(0.5f, 0.5f), new Vector2(0f, 566f), new Vector2(940f, 72f),
            ...
            MakeSeparator(cardGO.transform, 512f);

            // ── お題セクション ──
            var topicHeaderTmp = MakeLabel(cardGO.transform, "TopicHeader", "お題",
                new Vector2(0.5f, 0.5f), new Vector2(-380f, 472f), new Vector2(160f, 44f),
            ...
            var answerBgGO = MakeBox(cardGO.transform, "AnswerDisplayBox", AnswerBg, uiSprite,
                new Vector2(0f, 230f), new Vector2(940f, 120f));
            ...
            answererPanelGO.SetActive(false);

            var answerInputField = MakeInputField(answererPanelGO.transform, "AnswerInput",
                "回答を入力...",
                new Vector2(0.5f, 0.5f), new Vector2(0f, 102f), new Vector2(940f, 80f),
            ...
            var submitAnswerBtnGO = MakeButton(answererPanelGO.transform, "SubmitAnswerBtn",
                ...
                new Vector2(0f, 12f), new Vector2(360f, 80f),
                BtnGreen, Color.white, 40f, jpFont, btnYellow);

            MakeSeparator(cardGO.transform, 166f);

            // ── 予想セクション ──
            var predHeaderTmp = MakeLabel(cardGO.transform, "GuessSectionHeader",
                "みんなの予想",
                new Vector2(0.5f, 0.5f), new Vector2(0f, 120f), new Vector2(940f, 52f),
            ...
            scrollR.sizeDelta        = new Vector2(-40f, 460f);
            scrollR.anchoredPosition = new Vector2(0f, -24f);
            ...
            var guessInputField = MakeInputField(guesserPanelGO.transform, "GuessInput",
                "予想の数字 (1〜99)",
                new Vector2(0.5f, 0.5f), new Vector2(0f, -570f), new Vector2(560f, 80f),
            ...
            var submitGuessBtnGO = MakeButton(guesserPanelGO.transform, "SubmitGuessBtn",
                "予想を送る ▶",
                new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f),
                new Vector2(0f, -664f), new Vector2(360f, 80f),
            ...
            var finalDecideBtnGO = MakeButton(deciderPanelGO.transform, "FinalDecideBtn",
                "最終決定 ✓",
                new Vector2(0.5f, 0.5f), new Vector2(0.5f, 0.5f),
                new Vector2(0f, -664f), new Vector2(440f, 88f),
                BtnRed, Color.white, 44f, jpFont, btnYellow);

            // ── 観戦者パネル（ObserverPanel: stretch over card）──
            var observerPanelGO = new GameObject("ObserverPanel", typeof(RectTransform));
            observerPanelGO.transform.SetParent(cardGO.transform, false);
            StretchFull(observerPanelGO.GetComponent<RectTransform>());
            observerPanelGO.SetActive(false);

            var observerStatusLabelTmp = MakeLabel(observerPanelGO.transform, "ObserverStatusLabel",
                "観戦中...",
                new Vector2(0.5f, 0.5f), new Vector2(0f, 0f), new Vector2(800f, 120f),
                42f, TextMuted, FontStyles.Bold, jpFont);
            ...
            // ── チーム情報バー（MyInfoBar の直下）──
            var teamBarGO = new GameObject("TeamInfoBar", typeof(RectTransform));
            teamBarGO.transform.SetParent(panelGO.transform, false);
            teamBarGO.AddComponent<Image>().color = new Color(0.95f, 0.93f, 0.80f, 0.88f);
            var tbR = teamBarGO.GetComponent<RectTransform>();
            ...
            tbR.anchoredPosition = new Vector2(0f, -96f); // below MyInfoBar

            // teamALabel（左）
            var teamALabelTmp = MakeLabel(teamBarGO.transform, "TeamALabel", "チームA",
                new Vector2(0f, 0.5f), new Vector2(24f, 0f), new Vector2(340f, 56f),
                32f, new Color(0.15f, 0.35f, 0.75f), FontStyles.Bold, jpFont);
            teamALabelTmp.alignment = TextAlignmentOptions.MidlineLeft;

            // teamBLabel（右）
            var teamBLabelTmp = MakeLabel(teamBarGO.transform, "TeamBLabel", "チームB",
                new Vector2(1f, 0.5f), new Vector2(-24f, 0f), new Vector2(340f, 56f),
                32f, new Color(0.75f, 0.20f, 0.15f), FontStyles.Bold, jpFont);
            teamBLabelTmp.alignment = TextAlignmentOptions.MidlineRight;

            // activeTeamBadge（中央）
            var activeTeamBadgeTmp = MakeLabel(teamBarGO.transform, "ActiveTeamBadge", "▶ チームA の番",
                new Vector2(0.5f, 0.5f), new Vector2(0f, 0f), new Vector2(300f, 56f),
                34f, TextPrimary, FontStyles.Bold, jpFont);
            activeTeamBadgeTmp.alignment = TextAlignmentOptions.Center;
            ...
            // ── TeamGameController ──
            var ctrlGO = new GameObject("TeamGameController");
            ctrlGO.transform.SetParent(canvasGO.transform, false);
            var ctrl = ctrlGO.AddComponent<TeamGameController>();
            ...
            // チーム情報
            so.FindProperty("teamALabel").objectReferenceValue        = teamALabelTmp;
            so.FindProperty("teamBLabel").objectReferenceValue        = teamBLabelTmp;
            ...
            // 回答者パネル
            so.FindProperty("answererPanel").objectReferenceValue       = answererPanelGO;
            ...
            // 予想者パネル
            so.FindProperty("guesserPanel").objectReferenceValue       = guesserPanelGO;
            ...
            // 最終決定者パネル
            so.FindProperty("deciderPanel").objectReferenceValue       = deciderPanelGO;
            ...
            // 観戦者パネル
            so.FindProperty("observerPanel").objectReferenceValue        = observerPanelGO;
            so.FindProperty("observerStatusLabel").objectReferenceValue  = observerStatusLabelTmp;
            ...
            EditorSceneManager.SaveScene(
                ...
                "Assets/Scenes/TeamGame.unity");
            SceneSetupHelper.AddSceneToBuildSettings("Assets/Scenes/TeamGame.unity");
            Debug.Log("[TeamGameSceneBuilder] TeamGame シーンを作成しました");
        }
        ...
        static Sprite LoadSliced(string path, int left, int bottom, int right, int top)
        {
            ...
            if (ti == null) { Debug.LogWarning($"[TeamGameSceneBuilder] Not found: {path}"); return null; }
